...
<div class="mainMenuBand">Публикуване на обява в катеория <b>ОПТИКИ</b></div>
	<div class="mainField">
		<div class="addLogo" ><img src="{$smarty.const.WWW}templates/images/optika.gif" width="80" /></div>
	...
	{/if}
 	...
 	<form id="addingForm" name="addingForm" action="{$smarty.const.WWW}optic/save_ur_optic" method="POST" enctype="multipart/form-data">
	...
	<select class="aField" id="type_id" name="type_id" onchange="javascript: getOpticMarks();">
		{foreach $optic_types as $optic_type}
			<option value="{$optic_type->id}" {if $type_id == $optic_type->id}selected{/if}>{$optic_type->type}</option>
		{/foreach}
	</select>
    ...
    <select class="aField" id="mark_id" name="mark_id" onchange="javascript: getOpticModels();">
	</select>
...
<li><span class="redStar">*</span>Модел:<br />
	<select class="aField" id="model_id" name="model_id" onchange="javascript: getOpticSizes();">
	</select>
    ...
    <li><span class="redStar">*</span>Размер:<br />
    <select class="aField" id="size_id" name="size_id">
	</select>
	{if !empty($size_id_error)}
		<br><span class="error_message">{$size_id_error}</span>
	{/if}
	</li>
