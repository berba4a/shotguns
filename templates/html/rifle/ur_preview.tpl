...
<div class="mainMenuBand">Публикуване на обява в катеория <b>ПУШКИ - ПРЕГЛЕД НА ОБЯВАТА</b></div>
	...
	<div class="mainField">
	<div class="firstTitle">{$rifle->type->type} {$rifle->mark->mark} {$rifle->model->model} {$rifle->caliber->caliber} - {if $rifle->is_old} Използван {else} Нов {/if}</div>
	<div class="mainAdField">
		<div class="gallery">
			<ul class="gallery">
				<li><a href="{$smarty.const.WWW}{$rifle->images.0->image}" rel="lightbox[something]" title="my caption"><img class="adBigPic" id="bigPic" name="bigPic" src="{$smarty.const.WWW}{$rifle->images.0->image}" /></a></li>
				{foreach $rifle->images as $image}
		...
		<div class="adText"><span class="secondTitle">{$rifle->type->type} {$rifle->mark->mark} {$rifle->model->model} {$rifle->caliber->caliber} - {if $rifle->is_old} Използван {else} Нов {/if}</span></div><br />
<br />
<br />
<div  class="descrHeaders">
		<ul>
			...
			<li>Употреба:<span>{if $rifle->is_old} Използван {else} Нов {/if}</span></li>
			<li>Дата на публикуване:<span>{$rifle->created}</span></li>
			<li><hr style="margin-right:20px;" />Подробно описание:<hr style="margin-right:20px;" /><span>{$rifle->description}</span></li>
...
<div class="contactField">
	...
	<div style="margin-left:-20px;"  class="descrHeaders">
		...
		<ul>
			<li>Име за контакт:<span>{$rifle->user->real_name}</span></li>
			<li>Телефон за контакт:<span>{$rifle->user->phone}</span></li>
			<li>e-mail:<span>{$rifle->user->email}</span></li>
			<li>Местоположение:<span>{$rifle->city->city}</span></li>
...
{if !empty($save_preview)}
	<div style="text-align:center;margin-bottom:20px;">
		<input  type="button" id="btn_edit" name="btn_edit" value="Публикувай обявата" onclick="javascript: document.location = '{$smarty.const.WWW}rifle/ur_activate'"/>
		<input style="margin-left:200px;" type="button" id="btn_edit" name="btn_edit" value="Редактирай обявата" onclick="javascript: window.location = '{$smarty.const.WWW}rifle/ur_add/?edit=1';"/>
	</div>
{/if}
</div>